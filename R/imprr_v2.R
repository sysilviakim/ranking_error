...
#  J <- nchar(data[[main_q]][[1]])
  ...
  for (i in 1:n_bootstrap) {
    ...
    ## Marginal rank
    Y_rank_target <- data[target_item] %>% pull() 
    Y_rank_others <- list()
    for(k in 1:J_1){
    ...
    Y_pairwise <- list()                
    for(k in 1:J_1){
      compar <- data[other_items[k]] %>% pull() # Comparison item
    ...
    gg_topk <- do.call(rbind.data.frame, m_top) %>%
      select(estimate) %>%  
      mutate(outcome = paste0('Top-', '', 1:J_1),
             qoi = "top-k ranking",
             g_U = 1/J)
  ...
  df_random_summary <- df_random %>%
    summarize(mean = mean(est.p.random),
  ...
  df_qoi_summary <- do.call(rbind.data.frame, list_qoi) %>%
    ...
    arrange(bc_estimate) %>%
    summarize(mean = mean(bc_estimate),
