...
mdat$pid7 # explanatory variable

# Run Rank-order logit model ===================================================
# Estimating parameters (no weight)
m <- mlogit(
  ch ~ 1 | pid7 + ideo7 + partisan + male + age + race6 + educ + region,
  # Y ~ X_item | X_resp
  mdat, # Data
...
for (i in 1:7) {
  ...
  e_XB_party <- exp(v$`(Intercept):party` + v$`pid7:party` * i +
                    v$`ideo7:party` * .fix_pid +
                    v$`male:party` * .fix_male +
                    v$`age:party` *  .fix_age +
                    v$`educ:party` * .fix_edu)
  e_XB_race <- exp(v$`(Intercept):race` + v$`pid7:race` * i +
                     v$`ideo7:race` * .fix_pid +
                     v$`male:race` * .fix_male +
                     v$`age:race` *  .fix_age +
                     v$`educ:race` * .fix_edu)
  e_XB_reli <- exp(v$`(Intercept):religion` + v$`pid7:religion` * i +
                     v$`ideo7:religion` * .fix_pid +
...
ggdt_all %>%
  ...
  xlab("Partisanship (democrat - republican)") +
  ylab("Predicted Probability") +
  labs(caption = "Predictions for 40yo, white, male, with mid-level edu, moderate, in northeast") +
